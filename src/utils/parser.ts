function arrayPos(index: number) {
    return 2 * (index - 1);
}

function paramValue(index: number, params: (number | string)[]) {
    return params[arrayPos(index)];
}

function parseCondition(condition: string, index: number, params: (number | string)[]) {
    let operation = condition[0];
    let number = Number(condition.slice(1));
    let value = Number(paramValue(index, params));

    if (operation == '~') return params.length > arrayPos(number);
...
function parseLogic(str: string, params: (number | string)[], index: { value: number }) {
    let start = 1;
    let cursor = start;
    ...
    while (str[cursor] != ":" && bracket_level == 0) {
        if (str[cursor] == "{") bracket_level++;
        if (str[cursor] == "}") bracket_level--;
        cursor++;
    }

    let subString;
    if (checks) subString = str.slice(start, cursor);
    else subString = str.slice(cursor + 1);
...
function parseGeneric(str: string, params: (number | string)[], index = { value: 1 }) {
    let output = "";
    let cursor = 0;
    while (cursor < str.length) {
        if (str[cursor] == "[") {
            ...
            else console.log(`error : expected # or el but got ${str[cursor + 1]} instead at ${str.slice(cursor)}`);
        }
        else if (str[cursor] == '{') {
            let bracket_level = 1;
            cursor++;
...
function defaultParse(str: string) {
